...
{
    char *class_name;
    pr("bus=");prhex(bus);pr(" dev=");prhex(dev);pr(" func=");prhex(func);
    pr(" vendID=");prhex(p_pciDevice->option.vendorID);pr(" devID=");prhex(p_pciDevice->option.deviceID);
            
    class_name = GetPCIDevClassName(p_pciDevice->option.classCode);
    if (class_name)
        pr(" class_name=") pr(class_name);
