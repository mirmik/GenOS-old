...
{
	//print->print("\033[36m"); 
	print->print(MACHINE_NAME); print->print(":");
	//print->print("\033[0m");
}


	rl_terminal::rl_terminal(Stream* _strm,Print* prt,readline_t* _rl)
	{init(_strm,prt,_rl);}
	
   void	rl_terminal::init(Stream* _strm,Print* prt,readline_t* _rl)
	{strm=_strm; rl=_rl; mode=0;print=prt;}

#include "intcom/command_list.h"
   void	rl_terminal::new_line_handler()
   {Print* strmtemp=stdout;
	stdout=print;
...
//			hist_search (_HIST_DOWN);
			...
			if (rl->backspace ()) terminal_backspace();					
				if (rl->cmdlen != rl->cursor) {print->print ("\033[K");terminal_rewrite();}
			break;
...
void rl_terminal::terminal_move_cursor (int offset)
{
	if (offset==0) return;
	print->print ("\033[");
	print->print (abs(offset));
	if (offset > 0) print->print ("C");
	if (offset < 0) print->print ("D");
}
